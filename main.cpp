...
private:
	...
	map<pair<int, char>, pair<int, int>> lr_action_;
	map<pair<int, char>, pair<int, int>> lr_step;
	...
	while (not_enogh) {
		...
		for (auto rule : grammar_) {
			...
			for (auto c : first_[second_r[0]]) {
				if (c != '\0') {
					first_[first_r].insert(c);
				}
			}
			int i = 0;
			while (i < second_r.length() - 1 && first_[second_r[i]].count('\0')) {
			...
			if (grammar_symbols_.find(second_r[i]) != grammar_symbols_.end()) {
				if (i == second_r.length() - 1 && first_[second_r[i]].count('\0')) {
					first_[first_r].insert('\0');
				}
			}
...
void Algo::closing(set<Algo::element>& state) {
	...
	while (state.size() > size) {
		...
		for (auto item : state) {
			...
			if (curr_rule.second.length() == get<1>(item))
				continue;
			char B = curr_rule.second[get<1>(item)];
			for (int i = 0; i < grammar_.size(); i++) {
				...
				if (rule.first == B) {
					...
					for (auto c : first_[curr_rule.second[get<1>(item) + 1]]) {
						state.insert(make_tuple(i, 0, c));
					}
				}
			}
		}
...
set<Algo::element> Algo::step(const set<Algo::element>& state, char c) {
	...
	for (auto item : state) {
		...
		char B = curr_rule.second[get<1>(item)];
		if (B == c) {
			new_state.insert(make_tuple(get<0>(item), get<1>(item) + 1, get<2>(item)));
		}
	}
	closing(new_state);
	return new_state;
...
void Algo::grammar_check(int i) {
	for (auto rule : states_[i]) {
		if (grammar_[get<0>(rule)].first == start_symbol_ && get<1>(rule) == 1) {
			if (lr_action_.find(make_pair(i, '\0')) != lr_action_.end()) {
				if (lr_action_[make_pair(i, '\0')] != make_pair(2, -1))
					throw logic_error("Wrong grammar");
			}
			else
				lr_action_[make_pair(i, '\0')] = make_pair(2, -1);
		}
	}
	for (char c = -127; c < 127; c++) {
		for (auto rule : states_[i]) {
			if (get<1>(rule) < grammar_[get<0>(rule)].second.length()) {
				...
				if (!grammar_symbols_.count(a)) {
					for (int j = 0; j < states_.size(); j++) {
						if (step(states_[i], a) == states_[j]) {
							if (lr_action_.find(make_pair(i, a)) != lr_action_.end()) {
								if (lr_action_[make_pair(i, a)] != make_pair(0, j))
									throw logic_error("Wrong grammar");
							}
							else {
								lr_action_[make_pair(i, a)] = make_pair(0, j);
								break;
							}
						}
					}
				...
				if (a != c)
					continue;
				if (grammar_[get<0>(rule)].first == start_symbol_)
					continue;
				if (!grammar_symbols_.count(a)) {
					if (lr_action_.find(make_pair(i, a)) != lr_action_.end()) {
						if (lr_action_[make_pair(i, a)] != make_pair(1, get<0>(rule)))
							throw logic_error("Wrong grammar");
					}
					else {
						lr_action_[make_pair(i, a)] = make_pair(1, get<0>(rule));
					}
				}
			}
		}
...
void Algo::fit() {
	...
	for (int i = 0; i < states_.size(); i++) {
		grammar_check(i);
		for (char c = -127; c < 127; c++) {
			if (lr_action_.find(make_pair(i, c)) == lr_action_.end()) {
				lr_action_[make_pair(i, c)] = make_pair(3, -1);
			}
		}
	}
	for (int i = 0; i < states_.size(); i++) {
		for (auto a : grammar_symbols_) {
			for (int j = 0; j < states_.size(); j++) {
				if (step(states_[i], a) == states_[j]) {
					lr_step[make_pair(i, a)] = make_pair(0, j);
				}
			}
			if (lr_step.find(make_pair(i, a)) == lr_step.end()) {
				lr_step[make_pair(i, a)] = make_pair(1, -1);
			}
		}
	}
}
...
bool Algo::predict(string s) {
	...
	while (position <= s.length()) {
		char a = s[position];
		pair<int, char> t = helping_stack.back();
		if (lr_action_[make_pair(t.first, a)].first == 0) {
			position++;
			helping_stack.push_back(make_pair(lr_action_[make_pair(t.first, a)].second, a));
			continue;
		}
		if (lr_action_[make_pair(t.first, a)].first == 1) {
			pair<char, string> rule = grammar_[lr_action_[make_pair(t.first, a)].second];
			for (int i = 0; i < rule.second.length(); i++) {
				helping_stack.pop_back();
			}
			int q_new = helping_stack.back().first;
			char first_r = rule.first;
			if (lr_step[make_pair(q_new, first_r)].first == 0) {
				helping_stack.push_back(make_pair(lr_step[make_pair(q_new, first_r)].second, first_r));
			}
			else {
				break;
			}
		...
		if (lr_action_[make_pair(t.first, a)].first == 2) {
			return true;
		}
		if (lr_action_[make_pair(t.first, a)].first == 3) {
			break;
		}
	}
	return false;
